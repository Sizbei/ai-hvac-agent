...
- `importJobsFromFieldpulse(orgId, opts)` in `import/jobs.ts`:
  ...
  3. Resolve links: customer via `fieldpulseCustomerId` (Phase 3 guarantees it; if missing, create via the Phase-3 path inline), technician via `fieldpulseUserId` → `assignedTo`. **Self-heal rule (live-verified 2026-07-09):** FP `role` is an integer — role 4 = field technician (all 5 hold job assignments), role 1 = admin tier, but the FOUNDER (role 1) also runs jobs. When a job assignment references an FP user id with no native technician row, look the user up in the FP roster and create them as a technician (they demonstrably do field work) rather than dropping the assignment.

...
### Stage 5 — Reconcile the account-data verify gate across channels **[H, SECURITY] — 🟡 IN PROGRESS**
- **Confirmed (2026-06-24):** real gap — chat serves financial account intents (balance, membership-status) with NO verify step; voice gates them. (Resolved the `[[voice-chatbot-parity]]` contradiction: the gate did NOT ship on chat.)
- **Done this far:** extracted the pure, channel-agnostic decision engine `advanceVerify(intentId, state, zipAnswer, onFileZips) → {serve|ask|defer, verify}` into `account-verify.ts`, **exhaustively unit-tested** (8 cases: non-financial-no-fabricated-pass, passed/failed pass-through, first-ask, correct-ZIP→passed, wrong-ZIP re-ask vs lockout at `MAX_VERIFY_ATTEMPTS`, empty-on-file-ZIPs-never-pass). Zero behavior change yet (nothing calls it). This is the security core, made testable since the chat route POST has no unit harness.
- **Remaining (next tick):** wire `advanceVerify` into the chat ACCOUNT_LOOKUP block (read `metadata.verify`; for the `pending` case load on-file ZIPs from `customers`/`customerLocations` with `withTenant`+`decrypt`, degrade-safe; persist merged `{...meta, verify}`; reply with chat-worded ASK/DEFER — no DTMF copy). **CRITICAL (review finding):** also wire `preserveVerifyKey` into chat's metadata rebuilds at `route.ts:1286/1511` + the `1730-1797` re-read, or an intervening intake turn wipes the lockout → unlimited ZIP retries. Optionally refactor voice to call `advanceVerify` too (validates parity against voice's tests).
- **Verify:** advanceVerify unit tests ✅; route wiring via `tsc`+`build`+suite; no `hvac-knowledge.ts` change → eval not required; money-safety untouched (`buildAccountLookupReply` unchanged).

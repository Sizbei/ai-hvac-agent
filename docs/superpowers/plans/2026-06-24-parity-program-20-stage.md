...
### Stage 10 — HCP bulk operations + admin endpoint **[M, ops] — ✅ DONE 2026-06-24 (3 slices)**
- **Was real:** FP has chunked bulk-update; HCP had none. Option A (arbitrary status) was BLOCKED (HCP `updateJob` has no `work_status`), so shipped **Option B** (bulk `cancel`/`note`).
- **Done (sliced for the loop):** (1) `housecall-pro/bulk-types.ts` (`BulkJobOperation` action `note`|`cancel`) + pure `validateBulkOperations` (+7 tests). (2) `bulk-operations.ts` executor — bounded worker pool (order-preserving, no orphaned promises), per-item retry on transient errors, partial-success (`continueOnError`), error aggregation; each item runs under **`withHcpRateLimit`** → Stage 9's limiter now has its live consumer (+8 tests). (3) `/api/admin/integrations/housecall/bulk-update` route — `getAdminSession` 401, rate-limit, `Array.isArray` body guard, real `validateBulkOperations` → 400, `getHousecallClient` → NOT_CONFIGURED, `clientId=org:${orgId}` from the SESSION (not the body), summary + aggregated errors; GET returns a static capability note (`supportsStatusBulk:false`) documenting the HCP-only limitation (+5 route tests, incl. tenant-scope-from-session).
- **Verify:** ✅ 20 new tests; tsc + lint + full suite (2909) + build (route compiled). No DB writes (pure HCP API + session) → no neon-http/migration concern; no money/safety surfaces.

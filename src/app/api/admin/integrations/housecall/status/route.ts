/**
 * Housecall Pro connection status.
 *
...
export async function GET(): Promise<Response> {
  try {
    ...
    return successResponse({
      // An env fallback key means HCP is usable even before an explicit connect.
      configured:
        status.connected ||
        Boolean(process.env.HOUSECALL_API_KEY?.trim()),
      connected: status.connected,
      accountInfo: status.accountInfo,
    });
  } catch (error: unknown) {
    logger.error({ error }, "Failed to read Housecall Pro status");

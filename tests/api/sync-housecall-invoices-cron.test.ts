/**
 * HCP invoice reconcile cron — durability backstop. Fails closed without the
 * cron secret; when authorized, pulls each connected org's job invoices.
 */
import { describe, it, expect, beforeEach, vi } from "vitest";
import { GET } from "@/app/api/cron/sync-housecall-invoices/route";
...
vi.mock("@/lib/integrations/housecall-pro/invoice-sync", () => ({
...
const chain = (rows: unknown[]) =>
  ({ from: () => ({ where: () => Promise.resolve(rows) }) }) as never;
const reqWith = (auth: string | null) =>
  ({ headers: { get: () => auth } }) as unknown as Request;

beforeEach(() => vi.clearAllMocks());

describe("sync-housecall-invoices cron", () => {
  ...
  it("pulls each connected org's job invoices when authorized", async () => {
    ...
    mockedSelect
      .mockReturnValueOnce(chain([{ organizationId: "org-1" }]))
      .mockReturnValueOnce(chain([{ hcpJobId: "job-1" }]));
